...
def main():
	...
	_parser.add_argument("--update", dest="update", help="update the anubis framework", required=False)
	_parser.add_argument("--file", dest="file", help="binary path to analyze", required=False)
	_parser.add_argument("--install", dest="install", help="Install anubis-framework on the computer", required=False)
	_parser.add_argument('--uninstall', dest='uninstall', action='store_true',help='Uninstall anubis-framework on the computer')
	...
	try:
		...
		elif len(_args.update) > 0:
			print("actualziando anubis...")
		elif len(_args.install) > 0:
			print("instaladon anubis...")
		elif len(_args.uninstall) > 0:
			print("desinstalando anubis...")
		else:
